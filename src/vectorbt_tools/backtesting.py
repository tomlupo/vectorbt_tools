...
# %% helpers
def create_labels(index, drop_levels=-1):
    """
    Create labels from a MultiIndex by concatenating all levels except the dropped ones,
    including the level names in the label format.
    
    Parameters:
    - index: pd.MultiIndex, the multi-level index
    - drop_levels: level(s) to drop from the index (could be int, str, or list-like)
    
    Returns:
    - A pandas Index of formatted labels
    """
    # Drop the specified levels
    index_dropped = index.droplevel(drop_levels)
    
    # Concatenate the remaining levels with level names
    labels = []
    for values in index_dropped:
        label = "_".join([f"{level_name}-{value}" for level_name, value in zip(index_dropped.names, values)])
        labels.append(label)
...
def run(prices: pd.DataFrame, 
    ...
    Args:
        ...
        weights (dict or pd.DataFrame): Dictionary of strategy names to weight DataFrames or a pandas DataFrame with a MultiIndex of strategies and tickers.
        rebalancing_freq: Frequency at which the portfolio should be rebalanced. 
                          Can be None, an integer, a string ('D', 'W', 'M', 'Y'), or a list of dates.
    ...
    if isinstance(weights, pd.DataFrame):
        # Check if all tickers in weights DataFrame are present in prices DataFrame
        ...
        # Check if all tickers in weights dictionary are present in prices DataFrame
    ...
    direction = np.asarray(Direction.Both)
